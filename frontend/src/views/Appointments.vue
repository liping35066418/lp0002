<template>
  <div class="page-container">
    ...
    <el-table :data="list" border stripe v-loading="loading" :row-class-name="rowClassName">
      ...
      <el-table-column label="状态" width="110">
        <template #default="{ row }">
          <el-tag v-if="isUpcoming(row)" size="small" type="danger" effect="dark" class="blink-tag">即将到店</el-tag>
          <el-tag v-else size="small" :type="statusType(row.status)">{{ row.status }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="320" fixed="right">
        <template #default="{ row }">
          <template v-if="row.status !== '已完成' && row.status !== '已取消'">
            ...
            <el-button link type="success" @click="openCompleteDialog(row)" v-if="row.status === '服务中'">完成</el-button>
    ...
    <el-dialog v-model="dialogVisible" title="新建预约" width="700px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="宠物" required>
          <el-select v-model="form.pet_id" placeholder="选择宠物" style="width: 100%;" :disabled="!form.customer_id" @change="onPetChange">
    ...
    <el-dialog v-model="completeVisible" title="完成服务 - 填写服务备注" width="520px">
      <el-form :model="completeForm" label-width="90px">
        <el-descriptions v-if="currentAppointment" :column="2" border size="small" style="margin-bottom: 16px;">
          <el-descriptions-item label="宠物">{{ currentAppointment.pet_name }} ({{ currentAppointment.species }})</el-descriptions-item>
          <el-descriptions-item label="服务">{{ currentAppointment.service_name }}</el-descriptions-item>
          <el-descriptions-item label="客户">{{ currentAppointment.customer_name }}</el-descriptions-item>
          <el-descriptions-item label="美容师">{{ currentAppointment.staff_name || '-' }}</el-descriptions-item>
        </el-descriptions>
        <el-form-item label="服务备注">
          <el-input v-model="completeForm.service_remark" type="textarea" :rows="4" placeholder="如：毛结较多已剃除、皮肤有轻微红疹建议复查、情绪较紧张下次安抚等。下次该宠物到店时会自动提醒前台。" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="completeVisible = false">取消</el-button>
        <el-button type="success" @click="confirmComplete">确认完成服务</el-button>
      </template>
    </el-dialog>

    <el-dialog v-model="serviceRemarkVisible" title="上次服务备注提醒" width="500px">
      <el-alert
        type="warning"
        show-icon
        :closable="false"
      >
        <template #title>
          <div style="font-weight: 600; margin-bottom: 8px;">
            该宠物上次服务（{{ lastServiceRemark?.appointment_date }} {{ lastServiceRemark?.start_time }}
            - {{ lastServiceRemark?.service_name }}）有重要备注：
          </div>
          <div style="font-size: 14px; line-height: 1.8; padding: 8px 12px; background: #fffbe6; border-radius: 4px; border: 1px solid #faecd8;">
            {{ lastServiceRemark?.service_remark }}
          </div>
        </template>
      </el-alert>
      <template #footer>
        <el-button type="primary" @click="serviceRemarkVisible = false">我知道了</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const serviceRemarkVisible = ref(false)
const lastServiceRemark = ref(null)
const currentAppointment = ref(null)

const form = reactive({ customer_id: null, pet_id: null, service_id: null, appointment_date: '', start_time: '', remark: '' })
const completeForm = reactive({ service_remark: '' })

const upcomingIds = ref([])
...
const isUpcoming = (row) => upcomingIds.value.includes(row.id)

const rowClassName = ({ row }) => {
  if (isUpcoming(row)) return 'upcoming-row'
  return ''
}
...
const loadUpcoming = async () => {
  try {
    const res = await api.upcomingSoonAppointments()
    upcomingIds.value = res.data.upcomingIds || []
  } catch (e) {}
}
...
const onPetChange = async (petId) => {
  if (!petId) return
  try {
    const res = await api.petLastServiceRemark(petId)
    if (res.data) {
      lastServiceRemark.value = res.data
      serviceRemarkVisible.value = true
    }
  } catch (e) {
    console.error('加载上次服务备注失败', e)
  }
}
...
const confirmComplete = async () => {
  ...
  try {
    await ElMessageBox.confirm(`确定完成服务吗？`, '确认操作')
    const res = await api.updateAppointmentStatus(currentAppointment.value.id, {
      status: '已完成',
      service_remark: completeForm.service_remark
    })
...
onMounted(async () => {
  loadList()
  loadUpcoming()
  upcomingTimer = setInterval(loadUpcoming, 60000)
...
.upcoming-row {
  ...
  animation: upcomingBlink 1.5s ease-in-out infinite;
}
